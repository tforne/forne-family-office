...
export default function PortalBottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-white/70 bg-[rgba(248,250,253,0.88)] px-2 pb-[calc(0.55rem+env(safe-area-inset-bottom))] pt-2 shadow-[0_-18px_40px_-30px_rgba(15,47,87,0.25)] backdrop-blur-xl lg:hidden">
        ...
        {mobileItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex min-h-[64px] min-w-[74px] shrink-0 flex-col items-center justify-center rounded-[22px] px-3 py-2 text-center transition ${
                active
                  ? "bg-[linear-gradient(135deg,#123861_0%,#1b6fd8_100%)] text-white shadow-[0_18px_34px_-24px_rgba(15,47,87,0.65)]"
                  : "border border-transparent text-forne-muted hover:border-white/70 hover:bg-white hover:text-forne-ink"
              }`}
